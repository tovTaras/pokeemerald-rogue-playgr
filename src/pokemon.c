...
{
    ChangePersonality(&mon->box, personality); // Because of my changes (kleenexfeu) these two effectively the same thing
}
...
u32 GetBoxMonData3(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    {
        if(gSpeciesInfo[boxMon->species].forceTeraType != TYPE_NONE && gSpeciesInfo[boxMon->species].forceTeraType != 0)
            retVal = gSpeciesInfo[boxMon->species].forceTeraType - 1;
        else
        {
...
u16 GetDataCalculatedStats(struct Pokemon *mon, u8 ivid, u8 evid, u8 customstatid, u8 statIndex)
{
    ...
    baseStat = normalBaseStat + (4* (customStat)); // adding 15 to its basestats would actually increase 60 points of base stats
...
u16 GetMonAbility(struct Pokemon *mon)
{
    ...
    return (abilitycust != 0) ? abilitycust : GetAbilityBySpecies(species, abilityNum, otId);
}
...
{
    SetBoxMonData(mon, MON_DATA_NATURE, &nature);
}
...
// kinda nulled all that shit
void RandomlyGivePartyPokerus(struct Pokemon *party){}
u8 CheckPartyPokerus(struct Pokemon *party, u8 selection){return 0;}
u8 CheckPartyHasHadPokerus(struct Pokemon *party, u8 selection){return 0;}
